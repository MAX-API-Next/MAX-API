...
var (
	ErrSystemInstanceNotFound = errors.New("System instance not found.")
	ErrSystemInstanceOnline   = errors.New("Instance is still online and cannot be deleted.")
)
...
func DeleteStaleSystemInstance(nodeName string, now int64) error {
	...
	if err := DB.First(&instance, "node_name = ?", nodeName).Error; err != nil {
	...
	if now-instance.LastSeenAt <= SystemInstanceStaleAfterSeconds {
		return ErrSystemInstanceOnline
	}
