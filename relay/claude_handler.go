...
func ClaudeHelper(c *gin.Context, info *relaycommon.RelayInfo) (maxAPIError *types.MaxAPIError) {
	...
	if !model_setting.GetGlobalSettings().PassThroughRequestEnabled && !info.ChannelSetting.PassThroughBodyEnabled {
		effort, notes, err := reasoningcompat.ApplyClaudeSuffix(request, info.OriginModelName)
		if err != nil {
	...
	if !model_setting.GetGlobalSettings().PassThroughRequestEnabled &&
		...
		usage, maxApiErr := chatCompletionsViaResponses(c, info, adaptor, openAIRequest)
		if maxApiErr != nil {
			return maxApiErr
		}
	...
	if model_setting.GetGlobalSettings().PassThroughRequestEnabled || info.ChannelSetting.PassThroughBodyEnabled {
		...
		body, size, closer, err := relaycommon.PreparePassThroughJSONBody(storage, info.ChannelOtherSettings)
		if err != nil {
			return types.NewError(err, types.ErrorCodeConvertRequestFailed, types.ErrOptionWithSkipRetry())
		}
		if closer != nil {
