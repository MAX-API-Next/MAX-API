...
function migrateSystemConfigStorage(
  ...
  return {
    config: {
      ...
      logo: normalizeDefaultLogo(config.logo),
      currency: {
        ...DEFAULT_CURRENCY_CONFIG,
        ...(config.currency ?? {}),
      },
    },
    loadedLogoUrl: normalizeDefaultLogo(state.loadedLogoUrl),
  }
}
...
export const useSystemConfigStore = create<SystemConfigState>()(
  persist(
    ...
    }),
    {
      ...
      version: 2,
      migrate: migrateSystemConfigStorage,
      partialize: (state) => ({
        config: state.config,
